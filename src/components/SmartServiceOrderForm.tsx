  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (data.customerSelection === 'new' && data.new_customer_phone) {
        // 1. Buscar si el cliente ya existe por su teléfono
        const { data: existingCustomer, error: findError } = await supabase
          .from('customers')
          .select('id')
          .eq('phone', data.new_customer_phone)
          .single();

        if (findError && findError.code !== 'PGRST116') throw findError;

        if (existingCustomer) {
          // 2a. El cliente existe, usamos su ID
          customerId = existingCustomer.id;
          showSuccess("Cliente existente encontrado. La orden se asignará a su historial.");
        } else {
          // 2b. El cliente es nuevo, lo creamos
      ...
      // 3. Crear la orden de servicio con el ID del cliente (ya sea nuevo o existente)
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecciona un cliente existente o crea uno nuevo al instante.
          </DialogDescription>
        </DialogHeader>
              ...
              render={({ field }) => (
                <Tabs value={activeTab} onValueChange={(value) => {
                  ...
                  <TabsContent value="new" className="mt-4 space-y-2">
                    <div>
                      <Label htmlFor="new_customer_name">Nombre y Apellido</Label>
                      <Input id="new_customer_name" {...register("new_customer_name")} placeholder="Ej: Juan Pérez" />
                    </div>
                    <div>
                      <Label htmlFor="new_customer_phone">Teléfono</Label>
                      <Input id="new_customer_phone" {...register("new_customer_phone")} placeholder="Ej: 099123456" />
                    </div>
                  </TabsContent>
                </Tabs>
              )}
